...
describe('thenBy', () => 
{
    ...
    it('when called with a comparer then returns a collection sub-ordered with the comparer', () => 
    {
        let results = col2
            .orderBy(x => x.level1)
    ...
    it('when called after thenByDescending then returns a collection sub-ordered by the key', () => 
    {
        ...
        expect(results).toEqual([1, 4, 2, 3, 5]);
    });

    it('when called without a key selector then throws an exception', () => 
    {
        expect(() => { col1.orderBy(Linq.identity).thenBy(); }).toThrow();
    });

    it('when called with a non-function key selector then throws an exception', () => 
    {
        expect(() => { col1.orderBy(Linq.identity).thenBy('not-a-function'); }).toThrow();
    });

    it('when called with a non-function comparer then throws an exception', () => 
    {
        expect(() => { col1.orderBy(Linq.identity).thenBy(Linq.identity, 'not-a-function'); }).toThrow();
    });
});
